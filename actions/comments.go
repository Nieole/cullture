...
func (v CommentsResource) List(c buffalo.Context) error {
	...
	err := cache.Once(fmt.Sprintf("cache:comments:%v:%v:%v", c.Param("post_id"), c.Param("page"), c.Param("per_page")), list, func() (interface{}, error) {
		...
		if postID == "" {
			return nil, c.Render(http.StatusBadRequest, Fail("post_id不能为空"))
		}
		...
		if err := q.Eager("User").Where("post_id = ?", postID).Where("is_delete = ?", false).Order("created_at desc").All(comment); err != nil {
			return nil, err
		}
		return &ListResponse{
			Data:      comment,
