...
func (v PostsResource) List(c buffalo.Context) error {
	...
	if err != nil {
		...
		if err := q.Eager("Tags", "User", "Project").Scope(ByPage(c.Param("updated_at"))).Scope(ByProject(c.Param("project_id"))).Where("is_delete = ?", false).Order("updated_at desc").All(posts); err != nil {
			return err
		}
...
func MyList(c buffalo.Context) error {
	...
	if err != nil {
		// Retrieve all Posts from the DB
		if err := q.Eager("Tags", "User", "Project").Scope(ByPage(c.Param("updated_at"))).Scope(ByProject(c.Param("project_id"))).Where("user_id = ?", user.ID).Where("is_delete = ?", false).Order("updated_at desc").All(posts); err != nil {
			return err
		}
